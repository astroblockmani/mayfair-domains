...
contract Domains is ERC721URIStorage {
  ...
  constructor(string memory _tld) payable ERC721("Mayfair Name Service", "MYFR") {
    owner = payable(msg.sender);
    tld = _tld;
    console.log("%s name service deployed", _tld);
  }

  function register(string calldata name) public payable onlyOwner {
    ...
    string memory json = Base64.encode(
      abi.encodePacked(
        '{"name": "',
        _name,
        '", "description": "A domain on the Mayfair name service", "image": "data:image/svg+xml;base64,',
